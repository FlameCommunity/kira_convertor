...
def convert_to_dds(input_path, output_path):
	...
	if not os.path.exists(texconv_path):
		raise FileNotFoundError(f"[X] texconv.exe was not found in {texconv_path}. Ensure it is in the script folder.")
...
def get_format_choice(message):
	...
	while True:
		...
		print_colored("[X] Invalid option! Choose a valid number.", "red")
...
def convert_image(input_path, output_path, output_format, log_file):
	try:
		...
		log_message(log_file, f"[OK] Successful conversion: {input_path} -> {output_path}")
		print_colored(f"[OK] Successful conversion: {output_path}", "green")
	except Exception as e:
		log_message(log_file, f"[X] Conversion error: {input_path} -> {output_path} | {e}")
		print_colored(f"[X] Conversion error: {input_path} -> {output_path} | {e}", "red")
...
def main():
	...
	with open(log_path, "a", encoding="utf-8") as log_file:
		...
		if not files_found:
			log_message(log_file, f"[X] No files found with extension .{input_format} in '{input_folder}'.")
			print_colored(f"[X] No files found with extension .{input_format} in '{input_folder}'.", "red")
